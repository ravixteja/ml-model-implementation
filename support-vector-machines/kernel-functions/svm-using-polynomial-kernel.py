...
def polynomialKernel(r,d,X):
    # for i in range(X.shape[0]):
    #     print(X[i])
    kernel_value = (X@X.T + r) ** d
...
YX = matrix(polynomialKernel(2,4,X) * np.dot(Y,Y.T))
print(YX)
...
def predictClass(test_point):
    result = 0
    for i in range(X.shape[0]):
        result = result + alphas[i] * Y[i] * (np.dot(test_point,X[i] + 2) ** 4)
    result = result + bias
